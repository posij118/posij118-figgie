...
describe("endGame function", function () {
  ...
  it("recalculates ratings for a rated game", async function () {
    const gameName = "game";
    await startFourPlayerGame(gameName, true, false, false);
    const gameId = await getGameIdByGameName(null, gameName);
    ...
    assert.isAtLeast(ratingsDevsChanged.filter(didChange => didChange).length, 3);
	...
	assert.isAtLeast(ratingsVolsChanged.filter(didChange => didChange).length, 3);
